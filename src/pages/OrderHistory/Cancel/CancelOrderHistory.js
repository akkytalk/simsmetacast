...
function CancelOrderHistory(props) {
  const defaultProps = {
  ...
  const deleteStatus =
    props.login?.login?.user?.role == "admin"
      ? true
      : props.login?.login.user?.actions?.length > 0 &&
        props.login?.login.user?.actions[11]?.delete == 1
      ? true
      : false;
  ...
  const updateStatus =
    props.login?.login?.user?.role == "admin"
      ? true
      : props.login?.login.user?.actions?.length > 0 &&
        props.login?.login.user?.actions[11]?.update == 1
      ? true
      : false;
  const createStatus =
    props.login?.login?.user?.role == "admin"
      ? true
      : props.login?.login.user?.actions?.length > 0 &&
        props.login?.login.user?.actions[11]?.create == 1
      ? true
      : false;
  ...
  const rows = props.purchaseSalesIndent?.isLoading
    ? []
    : filter
    ? props.purchaseSalesIndent?.purchaseSalesIndent?.data?.filter((pur) => {
        return (
    ...
    : props.purchaseSalesIndent.purchaseSalesIndent?.data;
...
export default connect(mapStateToProps, mapDispatchToProps)(CancelOrderHistory);
