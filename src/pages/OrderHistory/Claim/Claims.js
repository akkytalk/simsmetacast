...
function Claims({ purchaseSalesIndentData, ...props }) {
  let data = {
  ...
  useEffect(() => {
    props.claimsGetData(data);
  }, []);

  const claimData =
    props.claims?.claims?.filter((c) => c.loading_detail_id == props.data.id)
      .length > 0
      ? props.claims?.claims.filter(
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    ...
    props.onPostClaimsData(data, user, toggle, setSubmitting);
    return;
  };
  ...
  return (
    <>
      ...
      <Modal className="modal-xl" isOpen={modal} toggle={toggle}>
        ...
        <ModalFooter>
          {props.claims?.isPostLoading && <LinerLoader />}
        </ModalFooter>
      </Modal>
    </>
  );
...
const mapDispatchToProps = (dispatch) => {
  return {
    claimsGetData: (data) => dispatch(actions.claimsGetData(data)),
    onDeleteClaims: (id, data) => dispatch(actions.deleteClaims(id, data)),
    onPostClaimsData: (data, user, toggle, setSubmitting) =>
      dispatch(actions.postClaimsData(data, user, toggle, setSubmitting)),
    updateClaimsData: (data, user, toggle, setSubmitting) =>
      dispatch(actions.updateClaimsData(data, user, toggle, setSubmitting)),
  };
};
